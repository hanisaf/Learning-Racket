...
;; Game -> Game
(define (update-all g)
  (make-game (update-invaders g) (update-missiles g) (advance-tank (game-tank g)) (add1 (game-ticks g))))

;; Game -> Invaders
(define (update-invaders g)
  (let ((invaders (game-invaders g)) (missiles (game-missiles g)) (tick (game-ticks g)))
    ;; Invader ListOfMissiles -> Boolean
    (define (intact-invader? i)
      (let ((any-collision? (lambda (m) (collide? i m))))
        (not (foldl or2 false (map any-collision? missiles))))) 
    ;; Invader -> Invader
    (define (advance-invader i)
      (cond [(> (invader-x i) WIDTH) (make-invader (- (invader-x i) INVADER-X-SPEED) (+ (invader-y i) INVADER-Y-SPEED) (- INVADER-X-SPEED))] ; when invader hits right edge it starts moving left
            [(< (invader-x i)     0) (make-invader (+ (invader-x i) INVADER-X-SPEED) (+ (invader-y i) INVADER-Y-SPEED) INVADER-X-SPEED)] ; when invader hits left edge it start moving right
            [else (make-invader (+ (invader-x i) (invader-dir i)) (+ (invader-y i) INVADER-Y-SPEED) (invader-dir i))]))
  ...
  (let ((invaders (game-invaders g)) (missiles (game-missiles g)) (tick (game-ticks g)))
    ...
    (define (intact-missile? m)
      (let ((any-collision? (lambda (i) (collide? i m))))
        (not (foldl or2 false (map any-collision? invaders)))))
    ;; Missile -> Boolean
    (define (in-screen? m)
      (< (missile-y m) HEIGHT))
    ;; Missile -> Missile
    ...
    (let ((intact-missiles (filter intact-missile? missiles)))         ; Discard missiles that hit a target
      (let ((in-screen-missiles (filter in-screen? intact-missiles)))  ; Discard missiles outside the screen
        (map advance-missile in-screen-missiles)))))                   ; Advance missiles
...
(define (handle-key g e)
  (cond [(key=? e "right") (make-game (game-invaders g) (game-missiles g) (tank-right (game-tank g)) (game-ticks g))]
        [(key=? e  "left") (make-game (game-invaders g) (game-missiles g) (tank-left  (game-tank g)) (game-ticks g))]
        [(key=? e     "a") (make-game (cons (random-invader) (game-invaders g)) (game-missiles g) (game-tank g) (game-ticks g))]
        [(key=? e     " ") (make-game (game-invaders g) (add-missile-to g) (game-tank g) (game-ticks g))]))
...
(define (any-invader-at-bottom? loi)
  (let ((at-bottom? (lambda (i) (>= (invader-y i) HEIGHT))))
    (foldl or2 false (map at-bottom? loi))))
...
(define (or2 a b) (or a b)) ; to pass when filtering lists
